...
public class ApiTests {
    ...
    public void getAllPostsByUser() {
        ...
        Assert.assertEquals(responseBody.length, expectedPostTDO.length);
        SoftAssertions softAssertions = new SoftAssertions();

        for (int i = 0; i < expectedPostTDO.length; i++) {
            softAssertions.assertThat(expectedPostTDO[i])
                    .isEqualToIgnoringGivenFields(responseBody[i], "_id", "createdDate", "author");
            softAssertions.assertThat(expectedPostTDO[i].getAuthor())
    ...
    public void getAllPostsByUserPath(){
        Response responsBody =
                given()
                        ...
                        .log().all()
                        .extract().response();
        List<String> titleList = responsBody.jsonPath().getList("title", String.class);
        List<Map> authorList = responsBody.jsonPath().getList("author", Map.class);
